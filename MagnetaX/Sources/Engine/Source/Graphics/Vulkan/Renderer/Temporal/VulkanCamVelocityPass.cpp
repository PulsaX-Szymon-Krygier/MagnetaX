...
#include <MX/Generated/Shaders/Temporal/VulkanShaderCamVelocityFrag.h>
...
namespace
{
    struct CamVelocityPushConstants
    {
        Matrix4f invViewProj;
        Matrix4f prevViewProj;
    };

    static_assert(sizeof(CamVelocityPushConstants) == 128);
}

bool VulkanCamVelocityPass::Create(const VulkanCamVelocityPassCreateInfo& createInfo)
{
    if (!createInfo.device || !createInfo.depthImage) return false;
    if (!createInfo.depthImage->GetImageView()) return false;
    ...
    imageInfo.imageView = createInfo.depthImage->GetImageView();
    ...
    pushConstRange.size = sizeof(CamVelocityPushConstants);
    ...
    pipelineInfo.fragmentShader = MX_GRAPHICS_VULKAN_SHADER_CAMVELOCITY_FRAG;
    pipelineInfo.fragmentShaderSize = MX_GRAPHICS_VULKAN_SHADER_CAMVELOCITY_FRAG_SIZE;
...
void VulkanCamVelocityPass::Destroy()
{
    pipeline.Destroy();
...
void VulkanCamVelocityPass::Record(const VulkanCamVelocityPassRenderInfo& renderInfo)
{
    ...
    CamVelocityPushConstants pushConstants{};
    pushConstants.invViewProj = renderInfo.invViewProj.Transposed();
    pushConstants.prevViewProj = renderInfo.prevViewProj.Transposed();

    vkCmdPushConstants(renderInfo.cmdBuffer, pipeline.GetPipelineLayout(), VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(CamVelocityPushConstants), &pushConstants);
